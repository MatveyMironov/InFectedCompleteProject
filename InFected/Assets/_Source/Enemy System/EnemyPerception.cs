...
namespace Enemy
{
    public class EnemyPerception
    {
        ...
        public PlayerSystem.Player PercievedPlayer { get; private set; }
        ...
        {
            if (TryDetectInRadius(percieverPosition, out PlayerSystem.Player player))
            {
        ...
        private bool TryDetectInRadius(Vector2 percieverPosition, out PlayerSystem.Player foundPlayer)
        {
            ...
            {
                if (detectedObject.TryGetComponent(out PlayerSystem.Player player))
                {
                    foundPlayer = player;
                    return true;
                }
        ...
        private bool CheckObstruction(PlayerSystem.Player player, Vector2 percieverPosition)
        {
